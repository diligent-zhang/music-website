...
public class SongController {
    ...
    public R addSong(SongRequest addSongRequest,@RequestParam("lrcfile") MultipartFile lrcfile, @RequestParam("file") MultipartFile mpfile) throws IOException {
    ...
    // 批量返回指定ID列表的歌曲
    @GetMapping("/song/batch")
    public R songOfIds(@RequestParam java.util.List<Integer> ids) {
        return songService.songOfIds(ids);
    }

    // 返回指定歌单的所有歌曲
    @GetMapping("/song/songList/detail")
    public R songOfSongListId(@RequestParam int songListId,
                              @RequestParam(defaultValue = "1") int page,
                              @RequestParam(defaultValue = "20") int pageSize) {
        return songService.songOfSongListId(songListId, page, pageSize);
    }
    // 返回指定歌手名的歌曲
    ...
    public R updateSongPic(@RequestParam("file") MultipartFile urlFile, @RequestParam("id") int id) throws IOException {
        return songService.updateSongPic(urlFile, id);
    }

    // 更新歌曲
    @PostMapping("/song/url/update")
    public R updateSongUrl(@RequestParam("file") MultipartFile urlFile, @RequestParam("id") int id) throws IOException {
