...
public class CollectServiceImpl extends ServiceImpl<CollectMapper, Collect> implements CollectService {
    ...
    public R collectionOfUser(Integer userId) {
        List<Collect> collects = collectMapper.selectList(new QueryWrapper<Collect>().eq("user_id", userId));
        return R.success("用户收藏", collects);
    }

    /**
     * existSongId 是实时性要求较高的判断（用户点收藏按钮时调用），不走缓存
     */
    @Override
