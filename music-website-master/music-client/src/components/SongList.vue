  ...
  setup(props) {
    ...
    const dataList = computed(() => {
      ...
      songList.value.forEach((item: any, index) => {
        // 优先使用后端返回的 singerName，兼容旧数据用 getSingerName 从歌名解析
        item["songName"] = item.singerName ? item.name : getSongTitle(item.name);
        item["singerName"] = item.singerName || getSingerName(item.name);
        item["index"] = index;
        list.push(item);
      });
      return list;
    ...
    function handleClick(row) {
      lyricDebug.reset();
      lyricDebug.step1_rowData(row);
      lyricDebug.step2_storeDispatch(row.lyric);
      ...
      router.push({ path: RouterName.Lyric + '/' + row.id });
    }
