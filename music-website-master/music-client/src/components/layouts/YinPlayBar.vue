<template>
  <div class="play-bar" :class="{ show: !toggle }">
    ...
    <div class="control-box">
      ...
      <div class="song-ctr">
        <span class="play-state-wrap" :class="{ 'single-repeat': playStateIndex === 1 }">
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      playStateList: [Icon.XUNHUAN, Icon.DANQUXUNHUAN, Icon.LUANXU],
      playStateIndex: 0,
      iconList: {
  ...
  methods: {
    ...
    // 上一首
    prev() {
      if (this.playState === Icon.DANQUXUNHUAN) {
        this.$store.commit("setIsPlay", false);
        this.$store.commit("setCurTime", 0);
        this.$nextTick(() => {
          this.$store.commit("setIsPlay", true);
        });
      } else if (this.playState === Icon.LUANXU) {
    ...
    toPlay(url) {
      if (url && url !== this.songUrl) {
        ...
        this.playMusic({
          ...
          name: song.name,
          lyric: song.lyric,
          singerName: song.singerName || '', // 加上这一行！
