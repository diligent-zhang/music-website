...
const HttpManager = {
  ...
  attachImageUrl: (url) => {
    ...
    // 处理音频文件 - 使用专门的音频接口
    if (url.startsWith('/song/') || url.includes('.mp3') || url.includes('.m4a')) {
  ...
  // 批量返回指定ID列表的歌曲
  getSongsByIds: (ids: number[]) => get(`song/batch?ids=${ids.join(',')}`),
  // 返回指定歌单的所有歌曲（JOIN查询，一次请求完成）
  getSongsBySongListId: (songListId: number, page = 1, pageSize = 10) =>
    get(`song/songList/detail?songListId=${songListId}&page=${page}&pageSize=${pageSize}`),
  // 返回指定歌手ID的歌曲
  ...
  // =======================> 排行榜 API
  // 获取排行榜数据
  getRankList: (type: string) => get(`rank/list?type=${type}`),
  // 更新歌曲播放次数
  updatePlayCount: (songId: number) => post(`rank/play`, { songId }),
  ...
  getMyTicketOrders: (userId: number) => get(`ticket/my-orders?userId=${userId}`),

  // =======================> 音符 API
  // 查询余额
  getYinbi: (userId: number) => get(`user/yinbi?userId=${userId}`),
  // 充值
  rechargeYinbi: (data: { userId: number; amount: number; transactionNo: string }) => post(`user/yinbi/recharge`, data),
  // 支付扣款
  payByYinbi: (data: { userId: number; amount: number }) => post(`user/yinbi/pay`, data),
