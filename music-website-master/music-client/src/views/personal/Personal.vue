  ...
  <div class="personal">
    <div class="personal-info">
      ...
      <div class="personal-msg">
        ...
        <div class="yinbi-box">
          <span class="yinbi-label">音符余额</span>
          <span class="yinbi-count">{{ yinbi }}</span>
          <el-button size="small" type="warning" :icon="Coin" plain round @click="rechargeVisible = true">充值</el-button>
        </div>
      </div>
    ...
    <!-- 音符充值弹窗 -->
    <yin-recharge-dialog v-model="rechargeVisible" @recharged="fetchYinbi"></yin-recharge-dialog>
  </div>
</template>
...
import YinRechargeDialog from "@/components/dialog/YinRechargeDialog.vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    async function fetchYinbi() {
      try {
        const res = (await HttpManager.getYinbi(userId.value)) as ResponseBody;
        if (res.success || res.data !== undefined) {
          store.commit('setYinbi', res.data?.yinbi ?? res.data ?? 0);
        }
      } catch { /* 忽略 */ }
    }
